...
func (m *Manager) Handler(w http.ResponseWriter, r *http.Request, router string) {
	...
	case proto.RouterGateway:
		m.gateways.connections.Handler(w, r, NewHandler(context.Background(), router, m.handlerGateway, m.gateways.clearChan))
	case proto.RouterWorker:
		m.workers.connections.Handler(w, r, NewHandler(context.Background(), router, m.handlerWorker, m.workers.clearChan))
